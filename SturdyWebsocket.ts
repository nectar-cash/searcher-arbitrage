...
const SHOULD_RECONNECT_FALSE_MESSAGE = 'Provided shouldReconnect() returned false. Closing permanently.'
const SHOULD_RECONNECT_PROMISE_FALSE_MESSAGE = 'Provided shouldReconnect() resolved to false. Closing permanently.'
...
export default class SturdyWebSocket implements WebSocket {
  ...
  constructor(public readonly url: string, protocolsOrOptions?: string | string[] | Options, options: Options = {}) {
  ...
  private handleOpen(event: Event): void {
    ...
    this.allClearTimeoutId = setTimeout(() => {
      ...
      this.debugLog(`WebSocket remained open for ${openTime} seconds. Resetting` + ' retry time and count.')
    }, allClearResetTime)
  }
  ...
  private handleClose(event: CloseEvent | undefined): void {
    ...
    if (typeof willReconnect === 'boolean') {
      ...
      willReconnect.then((willReconnectResolved) => {
        ...
        this.handleWillReconnect(willReconnectResolved, event, SHOULD_RECONNECT_PROMISE_FALSE_MESSAGE)
      })
    }
  }
  ...
  private handleWillReconnect(willReconnect: boolean, event: CloseEvent | undefined, denialReason: string): void {
    if (willReconnect) {
  ...
  private reestablishConnection(): void {
    const { minReconnectDelay, maxReconnectDelay, reconnectBackoffFactor } = this.options
    this.reconnectCount++
  ...
  private stopReconnecting(event: CloseEvent | undefined, debugReason: string): void {
  ...
  private dispatchEventOfType(type: string, event: any): boolean {
    ...
    if (type in this.listeners) {
      this.listeners[type].slice().forEach((listener) => this.callListener(listener, event))
    }
    return !event || !(event as Event).defaultPrevented
  }

  private callListener(listener: EventListenerOrEventListenerObject, event: Event): void {
...
function applyDefaultOptions(options: Options): Required<Options> {
  ...
  Object.keys(SturdyWebSocket.DEFAULT_OPTIONS).forEach((key) => {
    ...
    result[key] = value === undefined ? (SturdyWebSocket.DEFAULT_OPTIONS as any)[key] : value
  })
  return result
}
